...
def main() -> None:
    ...
    destination.add_argument(
        "--run-dir", type=Path, help="Experiment root; cache is RUN_DIR/cache"
    )
    ...
    output = (
        BaselineRunLayout.from_root(args.run_dir).cache
        if args.run_dir is not None
        else args.output
    )
    index = build_cache(
        args.source, args.split_csv, output, args.spacing, args.overwrite
    )
